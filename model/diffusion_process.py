...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "sqrt_alphas_target_cumprod", to_torch(np.sqrt(alphas_target_cumprod))
        )
        self.register_buffer(
            "sqrt_one_minus_alphas_cumprod", to_torch(np.sqrt(1.0 - alphas_cumprod))
        )
        self.register_buffer(
            "sqrt_one_minus_alphas_target_cumprod",
            to_torch(np.sqrt(1.0 - alphas_target_cumprod)),
        )
    ...
    def q_sample_target(self, y_target, t, noise=None):
        ...
        return (
            ...
            + extract(self.sqrt_one_minus_alphas_target_cumprod, t, y_target.shape)
            * noise
        )
    ...
    def p_losses(self, x_start, y_target, t, noise=None, noise1=None):
        ...
        y_noisy = self.q_sample_target(
            y_target=y_target, t=t, noise=noise1.to(y_target.device)
        )
